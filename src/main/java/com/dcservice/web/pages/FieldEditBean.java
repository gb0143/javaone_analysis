...
public class FieldEditBean extends BaseValidationBean implements Serializable {
    ...
    private void initField() throws NumberFormatException, IllegalAccessException, PersistenceBeanException {
	try {
	    field = DaoManager.query().from(QField.field).leftJoin(QField.field.options).fetch()
		    .where(QField.field.id.eq(Long.valueOf(getRequest().getParameter(RedirectHelper.ID_PARAMETER))))
