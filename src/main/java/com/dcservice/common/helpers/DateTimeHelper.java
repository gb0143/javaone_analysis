...
public class DateTimeHelper extends BaseBaseClass {
    ...
    private static DateFormat fromCalendar = new SimpleDateFormat("EEE dd MMM HH:mm:ss z yyyy", Locale.US);
    ...
    public static String toFormatedString(Date value, String format, Locale local, TimeZone timeZone) {
	...
	sb.append(((SimpleDateFormat) DateFormat.getDateInstance(DateFormat.MEDIUM, locale)).toPattern());
	sb.append(" ");
	sb.append(((SimpleDateFormat) DateFormat.getTimeInstance(DateFormat.SHORT, locale)).toPattern());
	sb.append(" Z");
	return sb.toString();
    }
	...
	public static <T extends BaseModel> List<SelectItem> listToCombobox(List<T> list, boolean showNotSelected) {
	...
	public static <T> List<SelectItem> fillList(Class<T> clazz, boolean showNotSelected) throws Exception {
	    return fillList(clazz, showNotSelected, false);
	}

	public static <T> List<SelectItem> fillList(Class<T> clazz, boolean showNotSelected, boolean showAllElement)
		throws Exception {
	    if (clazz.isEnum()) {
	...
	public static <T extends BaseModel> List<SelectItem> fillList(Class<T> clazz, List<Long> excludeList)
		throws HibernateException, PersistenceBeanException, IllegalAccessException {
	    return fillList(clazz, null, excludeList);
	}

	public static <T> List<SelectItem> fillList(Class<T> clazz, T[] excludeList, boolean showNotSelected,
		boolean showAllElement) throws HibernateException {
	...
	public static <T extends BaseModel> List<SelectItem> fillList(Class<T> clazz, Order order,
		List<Long> excludeList) throws HibernateException, PersistenceBeanException, IllegalAccessException {
	    return fillList(clazz, new ArrayList<Criterion>(0), order, excludeList);
	}

	public static <T extends BaseModel> List<SelectItem> fillList(Class<T> clazz, List<Criterion> ctiretions,
		Order order, List<Long> excludeList)
			throws HibernateException, PersistenceBeanException, IllegalAccessException {
	    List<SelectItem> list = new ArrayList<SelectItem>();
	    list.add(SelectItemHelper.getNotSelected());
	...
	public static <T extends BaseModel> List<SelectItem> fillList(Class<T> clazz, Order order,
		boolean addNotSelected) throws HibernateException, PersistenceBeanException, IllegalAccessException {
	...
	public static <T extends BaseModel> List<SelectItem> fillList(Class<T> clazz, Order order,
	    ...
	    /*
	     * for (T item : DaoManager.load(clazz, criteriaAlias, criterions,
	     * order)) { list.add(new SelectItem(item.getId(),
	     * item.toString())); }
	     */
	...
	public static <T extends BaseModel> List<SelectItem> fillList(List<T> list, boolean showNotSelected) {
	    return fillList(list, showNotSelected, false);
	}
	...
	public static List<SelectItem> fillList(Object[] array, boolean showNotSelected, boolean showAllElement) {
	    return fillList(array, null, showNotSelected, showAllElement);
	}

	public static List<SelectItem> fillList(Object[] array, Object[] exclude, boolean showNotSelected,
		boolean showAllElement) {
	    List<SelectItem> list = new ArrayList<SelectItem>();
	    if (showNotSelected) {
		list.add(SelectItemHelper.getNotSelected());
	...
	public static List<SelectItem> fillList(int from, int to, String prefix, boolean addNumbersPrefix) {
	    List<SelectItem> list = new ArrayList<SelectItem>();
	...
	private static String getNumberPrefix(Integer number) {
		...
		switch (Integer.parseInt(
			number.toString().substring(number.toString().length() - 1, number.toString().length()))) {
		case 1: {
		    return "st";
		}
		case 2: {
